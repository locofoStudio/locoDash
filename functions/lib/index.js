...
exports.healthCheck = exports.createPassHttp = exports.getVenuePasses = exports.generateCustomerPass = exports.createPass = exports.getVenuePassConfig = exports.saveVenuePassConfig = void 0;
...
admin.initializeApp();
// Set region to Hong Kong for better performance
const region = 'asia-east2'; // Hong Kong region
// Configure CORS
 ...
 */
exports.saveVenuePassConfig = functions.region(region).https.onCall(async (data, context) => {
    var _a;
    try {
        ...
        const venuePassTemplate = Object.assign(Object.assign({}, data), { updatedAt: new Date().toISOString(), updatedBy: ((_a = context.auth) === null || _a === void 0 ? void 0 : _a.uid) || 'emulator-user', templateType: 'venue_pass_design', status: 'active' });
 ...
 */
exports.getVenuePassConfig = functions.region(region).https.onCall(async (data, context) => {
    try {
        // Check authentication (skip for emulator testing)
        if (!context.auth && process.env.FUNCTIONS_EMULATOR !== 'true') {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
 ...
 */
exports.createPass = functions.region(region).https.onCall(async (data, context) => {
    var _a;
    try {
        // Check authentication (skip for emulator testing)
        if (!context.auth && process.env.FUNCTIONS_EMULATOR !== 'true') {
        ...
        const passRecord = {
            ...
            createdBy: ((_a = context.auth) === null || _a === void 0 ? void 0 : _a.uid) || 'emulator-user',
            status: 'generated'
        };
 ...
 */
exports.generateCustomerPass = functions.region(region).https.onCall(async (data, context) => {
    try {
        // Check authentication (skip for emulator testing)
        if (!context.auth && process.env.FUNCTIONS_EMULATOR !== 'true') {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
 ...
 */
exports.getVenuePasses = functions.region(region).https.onCall(async (data, context) => {
    try {
        // Check authentication (skip for emulator testing)
        if (!context.auth && process.env.FUNCTIONS_EMULATOR !== 'true') {
            throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
        ...
        const passesSnapshot = await db.collection('generatedPasses')
            ...
            .limit(100)
            .get();
        const passes = passesSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
        return {
            success: true,
            passes,
            count: passes.length
 ...
 */
exports.createPassHttp = functions.region(region).https.onRequest((req, res) => {
